...
void communicate (int sock)
{
    ...
    n = read(sock,outbuffer,1024);  ////////reading the choice of client
    printf("%s",outbuffer);
    return;
}
...
{	
	...
	{   
        ...
        if(count==0)
        return NULL;
        printf("Candidate List Succesfully Sent.\n");
        return data;
	}
...
{   int len=strlen(data);
    ...
    data = (char*)realloc(data,(len + len1 + len2)*sizeof(*data));
    strcat(data,data1);
    strcat(data,data2);
    strcat(data,"$");
